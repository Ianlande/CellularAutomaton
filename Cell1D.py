...
def make_table(rule):
    ...
    # 数组的数字转化成二进制然后倒序, rule=2则化成00000010, 倒序成01000000
...
class Cell1D:
    ...
    def start_random(self):
        ...
        # np.random.random(num)生成一个1行num列数组, 每个元素都是一个0-1的随机小数
        # self.array是int, 所以要四舍五入
    ...
    def start_string(self, s):
        ...
        # 输入为start_string("0111001"), 则array[0]为0111001
    ...
    def step(self):
        ...
        """
        从第0行开始, 第一行根据第0行的状态生成, 第二行根据第一行的状态生成, 以此类推
        以self.next作为位置标定, 以遍历整个array
        第一行用用专门的方法生成
        求i-1行(X)与window(Y)的相关函数F(X,Y) = c, 以c生成table作为第i行
        """
        a = self.array
        i = self.next
        window = [4, 2, 1]
